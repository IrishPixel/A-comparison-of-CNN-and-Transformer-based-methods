...
class SlideReader:
    ...
    def build_generator(self, size_px, size_um, stride_div, case_name, tiles_path,  category, fileSize, export = True, augment = False):
        ...
        for y in range(0, (self.shape[1]+1), stride):
            for x in range(0, (self.shape[0]+1), stride):
...
class Convoluter:
    ...
    def export_tiles(self, slide, pb, ignoredFile_list):
        ...
        print("!!!!!!!")
        print(whole_slide.export_folder)
        print("!!!!!!!")
...
if __name__ == ('__main__'):
    ...
    if isfile(args.slide):
        ...
        slide_list = []        
        for root, dirs, files in args.slide:
            for file in files:
                if ('.ndpi' in file or '.scn' in file or 'svs' in file or 'tif' in file or 'jpg' in file) and not 'csv' in file:
